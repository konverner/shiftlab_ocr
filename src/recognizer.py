...
class TransformerModel(nn.Module):
  def __init__(self, outtoken=len(ALPHABET), hidden=512, enc_layers=2, dec_layers=2, nhead=4, dropout=0.1):
...
class Recognizer:
  ...
  def run(self, img):
    ...
    src = t.Compose([t.ToPILImage(), t.Grayscale(1), t.ToTensor()])(torch.FloatTensor(img)).unsqueeze(0).to(DEVICE)
